...
- Updated API: [`overwolf.streaming`](../api/overwolf-streaming) [EXPERIMENTAL]:
  ...
  - [`overwolf.streaming.StreamAudioOptions.game`](../api/overwolf-streaming#gameaudiodevice-object) has been changed to [`overwolf.streaming.GameAudioDevice`](../api/overwolf-streaming#gameaudiodevice-object). The new field `filtered_capture` has been added, and can be used if `audioProcessCaptureSupported` returned true (this depends on the machine running the app, so make sure to always check it!).

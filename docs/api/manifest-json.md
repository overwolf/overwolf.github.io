...
| <a class="anchor" aria-hidden="true" id="disable_hardware_acceleration"></a>disable_hardware_acceleration | bool |  Disable GPU hardware acceleration, per window. Relevant only to native windows. </br>*Notes: Use this flag mainly for native windows that run as a second-screen with the game. It improves the performance of the game by reducing usage of the GPU while you are playing.*| 0.159  |

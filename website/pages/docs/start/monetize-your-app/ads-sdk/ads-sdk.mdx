...
The Overwolf advertising SDK, is a JavaScript library that allows app developers to show Ads inside their applications.
...
To get started with implementing and enabling ads, [contact us](/support/contact-us).
...
The Ads SDK works by linking an instance of `OwAd` to specific elements, that then become managed Ad containers.

</TabItem>
<TabItem value="ow-electron" label="ow-electron App">

The Ads SDK works by simply displaying an `<owadview/>` component, which then acts as a managed Ad container.

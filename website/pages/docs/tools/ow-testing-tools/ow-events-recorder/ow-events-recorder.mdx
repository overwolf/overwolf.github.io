...
and ensure that the your App reacts to it properly - Without even needing to run the game, much less replicate those exact scenarios.
...
Paired up with the different playback tools included in the [Overwolf Events Tester](/ow-events-tester), these can significantly help with automating tests.

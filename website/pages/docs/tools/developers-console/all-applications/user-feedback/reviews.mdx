...
When editing your team's reply to a review, you can use the `Discard` and `Update reply`, which correspond to the `Discard changes` and `Save` buttons of [The Footer Toolbar](../../#the-footer-toolbar).

Reviews will also often (but not always) contain text. when they do, it will also be shown inside the review's overview.

<Tabs>
<TabItem value="review-closed" label="Review" default>
...
<TabItem value="review-no-response" label="Review Without Response">

...
```
:::important
The virtualKey is an integer representation of the key. Please note that using Javascript, you should capture these using the "keydown" event. Using the "keypress" event will provide incorrect numbers. E.g. the B key provides virtualKey: 66 on "keydown" (correct), but virtualKey: 98 on "keypress" (incorrect).
:::

...
Read about [overwolf.media.replays.VideoSource](media/replays#videosource-object) for more info.  
Note: the overwolf.streaming API does **not** support secondary files for Webcam (means, the webcam rendered to a different video file), like in the [overwolf.media.replays](media/replays) API.  
...
`enable` Checks whether or not [`filtered_capture`](#gameaudiodevice-object) is supporetd on this system.

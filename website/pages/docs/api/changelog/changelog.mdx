...
Here you can find our [Developers Roadmap](https://trello.com/b/1V10E4IB/roadmap).
...
#### Game events
* Rainbow Six Siege
	* New "suffix" field was added in roster event. According to Ubisoft privacy rules, in case there is a value in this field it must be linked to the player's name field wherever it appears during the match - More details [here](games/events/rainbow-six#roster_xx-notes)

* Axie Infinity origin
	* New game support - More details [here](games/events/axie-infinity-origin)

* Valorant
	* New map support for Pitt - Pearl. More details [here](games/events/valorant)

* Dota 2
	* New support for Hero Pool detection for Captain’s Draft [here](games/events/dota2)

* Diablo 2
	* New game events. More details [here](overwolf-games-events-diablo2)
...
#### Platform
- Added the `tray_icon` flag to the [manifest](manifest). When provided, this icon will be used as the tray icon, rather than the launcher icon.
...
- Fixed issue where [`overwolf.utils.openUriInDefaultBrowser('ms-settings:display-advancedgraphics')`](utils#openurlindefaultbrowserurl) would not open the Microsoft Display settings.
- Fixed a bug involving [`overwolf.windows.flash()`](windows#flashwindowid-behaviorcallback).
- Fixed a bug where opening a native window could sometimes cause games to lose focus.
- Fixed a bug where calling [`overwolf.windows.bringToFront(false, callback)`](windows#bringtofrontgrabfocus-callback) or [`overwolf.windows.bringToFront(windowId, false, callback)`](windows#bringtofrontwindowid-grabfocus-callback) could sometimes cause a game to lose focus.

## Version 0.199 (June 2022)

#### Docs
- Updated the documentation to reflect the new API changes listed below.
- Added documentation of the [overwolf.games.GameInfoType](games#gameinfotype-enum) Enum.

#### API
- Updated API: [`overwolf.streaming`](streaming) [EXPERIMENTAL]:
  ...
  - [`overwolf.streaming.getCapabilities`](streaming) - New method for obtaining all streaming capabilities in one place, for both video and audio.
    - Returns a callback with [`overwolf.streaming.StreamingCapabilities`](streaming).
    - Of particular note is the new `audioProcessCaptureSupported` flag, that lets us know if `filtered_capture` can be enabled.
  - [`overwolf.streaming.StreamAudioOptions.game`](streaming) has been changed to [`overwolf.streaming.GameAudioDevice`](streaming). The new field `filtered_capture` has been added, and can be used if `audioProcessCaptureSupported` returned true (this depends on the machine running the app, so make sure to always check it!).

- New API: [`overwolf.games.tracked.getRunningGameInfo`](games/tracked#getrunninggameinfocallback) - New method for obtaining game info from tracked games, similar to [`overwolf.games.getRunningGameInfo`](games).
  - This method will only work if `tracked` and `track_all_games` are both set to true in the [app manifest](manifest#launch_event_settings-array), but it will work on **all** games, regardless of whether they're supported or not.
...
#### Bug Fixes
- [`overwolf.games.getRunningGameInfo`](games) will no longer return a monitor handle of 0 in certain edge cases.
...
* [Dota](games/events/dota2)
  * New info update team_score with the score of each team during the game more info [here](games/events/dota2)
* [Rocket League](games/events/rocket-league)
  * Support dropshot game mode
* [Call of duty - Vanguard](games/events/vanguard)
...
* [Fortnite](games/events/fortnite)
  * Add new info update of roster - see notes [here](games/events/fortnite#roster-note)
  * Add the name of the knocked out player in knockout event- see notes [here](games/events/fortnite#knockout-note)
...
- New API: [`overwolf.utils.getClientInfo`](utils) - a method for obtaining the Overwolf client's uptime, as well as install date.
- Updated API: [`overwolf.windows.onAltF4Blocked`](windows) - is now invoked for all windows in the app as soon as it occursed on any window. Event result now also contains the name of the window that caused the invocation.
...
#### Game events
* [Teamfight Tactics](games/events/tft)
  * Add new info update "augments" for the list of available augments feature in the match - see notes [here](games/events/tft#me)
* [Fortnite](games/events/fortnite)
  * Add new info update for ping and skirmish data - see notes [here](games/events/fortnite)

#### API
- New API: [`overwolf.games.tracked.onUnsupportedExecuted`](games/tracked#onunsupportedexecuted) - a listener function that will trigger when a process that is defined as "Unsupported" in the Gameslist launches. It's important to note that it will only trigger if the app has defined `"track_all_games":true` in the `manifest.json`.

#### Platform
- New `manifest.json` flag:
  - `track_all_games` is a new optional manifest flag located under the [`launch_events`](manifest#launch_event_settings-array) property, 
 ...
 * [Minecraft](games/events/minecraft)
   * New info_update
     * counters - ping
     * player_X - list of active players in the game

 * [Rainbow Six Siege](games/events/rainbow-six)
   * Add player_id in roster - [here](games/events/rainbow-six#roster)

<div>{/* * [World of Warcraft](games/events/wow)
  * Add new event for Addons varaibles var_addon_xx */} </div>

 * [Halo Infinite](games/events/halo-infinite)
...
#### API
- [`overwolf.utils.checkForClientUpdates()`](utils) - This new method can be used to check whether an Overwolf update is available.  
 ...
 -   [`overwolf.windows.getWindow()`](windows) - This methods returns 
...
* [Hunt: Showdown](games/events/hunt-showdown)
  * New supported game
  * New game_info:
    * scene
* [Rainbow Six Siege](games/events/rainbow-six)
  * Add new privacy information about the local player - [event "me"](games/events/rainbow-six#me)
...
* [League of Legends](games/events/lol)
  * New feature in:
    * jungle_camps
      * baron & rift_herald icon_status remains at zero value at all times (Full information [here](https://github.com/overwolf/overwolf.github.io/blob/source/docs/games-events-lol.md#jungle_camp-note)).
...
* [World of Warcraft](games/events/wow)
  * New feature in:
    * addons
* [Halo Infinite](games/events/halo-infinite)
  * New supported game.
...
* New manifest flag for running background tasks when a user uninstalls an app like removing files, logs, images, etc...
  ...
  * More info [here](manifest#uninstall_window-object).
...
* [Rocket League](games/events/rocket-league)
  * New info updates:
    * car_look_inventory
    * trade_my_inventory
* [Warframe](games/events/warframe)
...
* Added support for file:// URI scheme in [overwolf.media.videoscreateVideoComposition()](media/videos#createvideocompositionsourcevideourl-segments-callback).  
...
* [MTGA](games/events/mtga)
  * New feature under:
    * game_info
* [Warzone](games/events/warzone)
  * New update & note for:
    * roster
* [New World](games/events/new-world)
  * New supported game.
* [Eternal Return](games/events/eternal-return)
  * New supported game.
* [Hearthstone](games/events/hearthstone)
  * New info update:
    * arena
...
* New [IO sample app](io#sample-app): Demonstrates how to open and load a file to your app, Display it, Add some content, and Write it back to the file.
    
#### API

* New flag for the [OverlayInfo](games#overlayinfo-object) object: [isFullScreenOptimizationDisabled](games#isfullscreenoptimizationdisabled-note). The OW overlay is not visible in full-screen mode for games that require an exclusive mode to interact with the OW app's windows (like Destiny 2, CSGO, and warzone). This indication lets you show the user a desktop notification to switch from full screen to windowed or borderless mode or enable full-screen optimization.

* New low disk space warnings when we are recording: 
  * If you are using the Streaming API, [onStreamingWarning](streaming#onstreamingwarning) event is fired, with the error text `disk_space_warning`.
  * If you are using the Replays API,  [onCaptureWarning](media/replays#oncapturewarning) event is fired, with the error text `disk_space_warning`.
  * It will show in the [obs log](/topics/understanding-ow-logs/logs-obs) as: `On disk space warning...`.

* New [encoding](io#encoding-enum) param to [overwolf.io.listenOnFile()](io#listenonfileid-path-option-callback).

#### Platform

* Support capturing rate of up to 120 FPS.  
  You have the option to either change the FPS in the Overwolf settings or App Creators can use it with our [Streaming API](streaming).
...
* [overwolf.window.setZoom()](windows#setzoomwinzoomfactorowid-windowid) can now be applied to a specific window and not to all of the app windows.
...
## Version 0.176 (August 2021)

* Note that iterations 176 to 177 were combined into a single iteration. 

#### API

* Toast Notification - Overwolf apps can use the newly implemented API, [overwolf.notification](notifications), to show users native Windows notifications (those that pop up on the right bottom side of your screen).
  * [showToastNotification(ToastNotificationParams, callback)](notifications#showtoastnotificationargs-callback) - This self explanatory function shows the native Windows notification once called.
  * [onToastNotification](notifications#ontoastnotification) - An event listener that triggers when a notification was interacted with.
  * New Enums: [AppLogoCrop](notifications#applogocrop-enum), [ToatsEventType](notifications#toatseventtype-enum), [ToastEventError](notifications#toasteventerror-enum).
* Dedicate video capture option for specific games (CSGO, Warzone, Destiny 2) that requires [exclusive mode](/topics/best-practices/exclusive-mode) to interact with the OW windows: up until now when we recorded these games in windowed mode, it resulted in a recording of the whole desktop. We now provide the option to record only the game window.
  * This feature is disabled by default, but you can enable it with the new `game_window_capture` property that you can find in the video settings object in the [replays API](media/replays#replayvideooptions-object) or the [streaming API](streaming#game_window_capture-notes).  
  * New `is_game_window_capture` properties added to [onStartStreaming](streaming#onstartstreaming) and [onReplayServicesStarted](media/replays#onreplayservicesstarted) to inform the app Creator if this new capturing feature was used or not on the current OW window.

#### Bug fixes

* Subscription in-app login form fix.
* Overwolf will no longer crash when merging too many files while using [createVideoCompositionFiles()](media/videos#createvideocompositionfilesfiles-outputfile-callback).
...
* New sample apps to make the OW creators life easier: [login with Twitch/Steam sample app](/topics/integrations/login-with-twitch), [Replays API sample app](media/replays#sample-app), [Streaming API sample app](streaming#sample-app), and [Channels API sample app](/topics/developers-console/app-channels#app-channels-sample-app).
 ...
 * New manifest flag: Now you can set the [window style](manifest#windows-style) as `InputPassThrough` through the manifest, and not just with the [overwolf.windows.setWindowStyle()](windows#setwindowstylewindowid-style-callback) function.
 * New enum: [overwolf.profile.subscriptions.inapp.enums.Theme](profile/subscriptions/inapp#theme-enum), to use with [overwolf.profile.subscriptions.inapp.show()](profile/subscriptions/inapp#showplanid-theme-callback).  
 * [overwolf.windows.changeSize(windowId, width, height, callback)](windows#changesizewindowid-width-height-callback) is now obsolete, replaced by [overwolf.windows.changeSize(changeSizeParams, callback)](windows#changesizechangesizeparams-callback).
...
* Added `processId` to [overwolf.games.getRunningGameInfo](games#getrunninggameinfocallback) and [overwolf.games.onGameInfoUpdated](games#ongameinfoupdated).
* Coexistence API - Event to indicate if there is another overlay library running + if Overwolf doesn't manage to render anything, allow the app to give a relevant notification
  * Added a new object, [overwolf.games.OverlayInfo](games#overlayinfo-object),  with info about the current out of process overlays.
  * Added a new [GameInfoChangeReason](games#gameinfochangereason-enum) enum to the [overwolf.games.onGameInfoUpdated](games#ongameinfoupdated) event,with some info about the hooking error reason. (if an hooking error has occurred).
  * Added a new [KnownOverlayCoexistenceApps](games#knownoverlaycoexistenceapps-enum) enum with a list of known apps causing overlay coexistence issues with OW.
  * New API: [overwolf.games.getLastRunningGameInfo()](games##getlastrunninggameinfocallback) - returns the last played gameinfo (when no game is currently running). 
  * new API: [overwolf.settings.games.setAutoLaunchEnabled](settings/games#setautolaunchenabledgameclassid-callback): allows apps to control what games they auto-launch for following user gesture (similar to performUpdate).
  * New API: [overwolf.os.tray.destroy](os/tray#destroy) - Added a new method for removing an extension tray icon and menu immediately.
...
* [GameInfoUpdatedEvent](games#gameinfoupdatedevent-object) Object - fixed the issue that runningChanged is not being updated once a game starts running.
...
* [Football Manager2021](games/events/fm2021)
...
* **App Channels:**  
  ...
  * API: [The overwolf.settings.setExtensionSettings](settings#setextensionsettingsextensionsettings-callback) function now accepts a new property in its [GeneralExtensionSettings](settings#generalextensionsettings-object) object called `channel`, this will set the app to a specific channel.
* **In-app Subscriptions flow:**  
  ...
  * New API, [overwolf.profile.subscriptions.inapp](profile/subscriptions/inapp): Allow apps to launch the subscription window inside their app and change subscription through the app (includs new permission: "subscription").
  * API: New function introduced to return additional details about the subscribed user, such as expiry date and subscription state:  
    [overwolf.profile.subscriptions.getDetailedActivePlans()](profile/subscriptions#getdetailedactiveplanscallback).
  *  new enum with the 3 available subscription states: active, cancelled, and revoked: [overwolf.profile.SubscriptionState](profile/subscriptions#subscriptionstate-enum).
  * [overwolf.profile.getCurrentUser](profile#getcurrentusercallback) - this API now also returns the user’s display name.
...
* Fix [overwolf.utils.openUrlInOverwolfBrowser](utils#openurlinoverwolfbrowserurl): allow to open tab and redirect back to extension when origin URL is at 'externally_connectable' and to tab was open from the same extension.
...
* [Rocket League](games/events/rocket-league)
  * New info update under:
    * game_info
* [Apex Legends](games/events/apex-legends)
  * New "Arena" game mode will be enabled in the near future.
* [Valheim](games/events/valheim)
  * New updates and events under:
    * game_info
    * kill


## Version 0.169 (April 2021)

#### API

* Improve OBS encoders support detection using the [overrwolf.streaming.getStreamEncoders()](streaming#getstreamencoderscallback) API (we added new error state for unsupported encoders [EncoderData](streaming#encoderdata-object) object).
* Added the following properties to [overwolf.io.paths](io/paths):
...
* New event: [overwolf.streaming.onSupportedEncodersUpdated](streaming#supportedencodersupdatedevent-object) -  Fired upon support encoder list updated.
...
* [Valehim](games/events/valheim)
  * New supported game
* [Dota](games/events/dota2)
  * New info update
    * game_mode
* [TFT](games/events/tft)
...
## Version 0.168 (April 2021)

Note that iterations 167 to 168 were combined into a single iteration.  

#### API

* You can now detect if the [Windows 10 Hardware-Accelerated GPU Scheduling](/topics/best-practices/video-capture#windows-10-hardware-accelerated-gpu-scheduling-notice) feature is enabled by calling [overwolf.utils.getSystemInformation()](utils). The callback object contains the **HAGSEnabled** property that can be true or false.  Note that the user can set the HA feature on or off therough the client settings.
* Close overwolf when closing an app: apps can now add the new 'Shutdown' permissions to it's manifest, then call [overwolf.settings.setExtensionSettings()](settings#setextensionsettingsextensionsettings-callback) with "exit_overwolf_on_exit: true".
  * **Note: This option shouldn’t be used without Overwolf’s permission**.
* Social API: [overwolf.social.youtube.performLogin()](social/youtube#performlogoutcallback) now opens in default browser and using pubsub to update client.
* [overwolf.util.openFilePicker()](utils#openfilepickerfilter-callback): now returns both url and path to file.  
...
## Version 0.166 (March 2021)


#### API

* The [disable_cleanup](manifest#disable_cleanup) flag in the manifest.json no longer deletes the app's files in the `%Localappdata%/Overwolf/Extensions` folder.
...
## Version 0.165 (February 2021)

Note that iterations 163 to 165 were combined into a single iteration.  

#### API

* [overwolf.media.videos.addWatermark()](media/videos#addwatermarksourcevideourl-watermarkurl-watermarkparams-callback) watermark file now supports absolute/local path to a file in the extension directory.  
...
* Increase max log file item to upload from 1 Mb to 3Mb (When using [uploadClientLogs()](utils#uploadclientlogscallback)).
...
* Fixed: If a hotkey is defined in the manifest with no default key binding, [hotkeys.get()](settings/hotkeys#getcallback)/attempting to set the hotkey would result in an error.
* The setting exit_overwolf_on_exit (can be set through [overwolf.settings.setExtensionSettings()](settings#setextensionsettingsextensionsettings-callback)) no longer closes Overwolf when an app that was using that setting has crashed, however, it will still close Overwolf if the user has dismissed the crash notification or didn't click on the "Relaunch" button in that same notification.
...
* Allow apps to add/remove their hotkeys right from the app (using a new API) without opening the OW client UI. You can assign only hotkeys that pre-defined in your manifest.
  * Use the new [overwolf.settings.hotkeys.assign()](settings/hotkeys#assignhotkey-callback) and  [overwolf.settings.hotkeys.unassign()](settings/hotkeys#unassignhotkey-callback) APIs.
* Reddit presents new required param for sharing: added a new share param called [Flair](social/reddit#flair-object),  and a new function: [overwolf.social.reddit.getSubredditFlairs()](social/reddit#getsubredditflairssubredditname-callback).
* Add to [GameInfoUpdatedEvent](games#gameinfoupdatedevent-object):  
...
* Added a new API namespace: [overwolf.cryptography](cryptography).
  * Currently supports two functions: *encryptForCurrentUser()* and *decryptForCurrentUser()*.
* Added [overwolf.games.inputTracking.init()](games/input-tracking#initcallback) for using *getMousePosition* without pre-register to events.
...
* [Rocket League](games/events/rocket-league)
  * New events & updates:
    * death
    * action_points
* [Call of Duty: Warzone](games/events/warzone)
  * New events & updates:
    * game_mode
    * game_map

## Version 0.159 (December 2020)

#### API

* [overwolf.windows.obtainDeclaredWindow()](windows#obtaindeclaredwindowwindowname-callback) will now also return monitorId (where applicable) - allowing one window (background) to know on which monitor another window exists. 
* Remove app indexedDB upon uninstall.
* [overwolf.streaming.start](streaming#startsettings-callback) now support undefined settings.
* WebCam support: 
  ...
  * New API to retrieve connected WebCams: [overwolf.media.getWebcams](media#getwebcamscallback).
* [overwolf.windows.setDesktopOnly()](windows#setdesktoponlywindowid-shouldbedesktoponly-callback) - This function is now obsolete.
...
* Valorant.
  * Added new update: **pseudo_match_id**  under [match_info](games/events/valorant#match_info).
* COD: Warzone.
  * Added new update: **pseudo_match_id**  under [match_info](games/events/warzone#match_info).
* Overwatch.
  * Added new update: **pseudo_match_id**  under [match_info](games/events/overwatch#match_info).

## Version 0.158 (November 2020)

#### API

* [disable_hardware_acceleration](manifest#disable_hardware_acceleration) new manifest flag:
...
* New API: [overwolf.games.inputTracking.onMouseWheel](games/input-tracking#onmousewheel): Fired a mouse wheel event.
* New window manifest flag [restrict_to_game_bounds](manifest#restrict_to_game_bounds):  
...
* URI protocol: allows apps to open by using a URI protocol that can be passed via the command-line or a web address.  
  Just add the new [url_protocol](manifest#url_protocol) flag to your app's manifest, to create a link to your app.  
...
* [Final Fantasy XIV](games/events/ff-xiv)
  * New info update under:
    * match_info

## Version 0.157 (November 2020)

#### API

* [overwolf.utils.openFilePicker](utils#openfilepickerfilter-initialpath-callback) - now accepts multiple selection of files.  The selected files will be returned on the callback as an array.
* [overwolf.io.listenOnFile](io#listenonfileid-path-option-callback) - add an indication that the watching on file started successfully.
...
## Version 0.156 (October 2020)

#### API

* [overwolf.windows.onScreenPropertyChanged](windows#onscreenpropertychanged) event:  
...
* [overwolf.extensions.current.getExtraObject](extensions/current#getextraobjectname-callback): Improve the robustness of the API. Write to log and return a callback.
* [overwolf.utils.getSystemInformation](utils#getsysteminformationcallback): return if Windows 10 version 2004 *Hardware-Accelerated GPU Scheduling* is enabled. 
...
* Multiple audio tracks: It is now possible to record videos that support more than one audio track, using the new flag for [stream settings](streaming#streamsettings-object): **separate_tracks": bool**.  
...
## Version 0.155 (September 2020)

Note that iterations 154 and 155 were combined into a single iteration.  

#### API

* [overwolf.utils.placeOnClipboard()](utils#placeonclipboarddata) and [overwolf.utils.getFromClipboard()](utils#getfromclipboardcallback) reimplemented so that we tackle the coexistence issues with AutoHotKey script.

* [overwolf.games.inputTracking.getActivityInformation()](games/input-tracking#getactivityinformationcallback) - remove the 5 minutes min time for getting a result.  
...
* New flag for [replay](media/replays#replayvideooptions-object) \ [streaming](streaming#streamvideooptions-object) video settings object: **use_app_display_name**.  
  If this flag in on, when creating a video capture folder, it will use the app "short name" as the folder name, instead of using the app name from the manifest. (the app "short name" is defined in the [dock_button_title](manifest#meta-dock) manifest flag).
...
New manifest flag [max_rotation_log_files](manifest#max_rotation_log_files) - for app Log rotaion control. It allow to increase app log's file rotation (up to 40).
    
* New feature: Set window zoom level - [overwolf.window.setZoom()](windows#setzoomwinzoomfactorowid-windowid).  
...
* The [overwolf.media.audio](media/audio) is now considered obsolete, and will display the relevant warning in the console when using it.
...
* WebServer: [overwolf.web.webserver](web/webserver) now allow CORS.  
  ...
  It's useful for case when you need to pass some data (ex: authentication token) from your website to Overwolf app. More info [here](web/webserver).
...
* [Hearthstone](games/events/hearthstone) - "match_end" now includes a match outcome value.d
* [League of Legends](games/events/lol#live_client_data) - new event - "port". Random port that comes from Riot

## Version 0.153 (August 2020)

Note that iterations 150 to 153 were combined into a single iteration.  

#### API

* [overwolf.utils.openUrlInDefaultBrowser()](utils#openurlindefaultbrowserurlopenurloptions) - Now allows extra behaviour. Right now "skip_in_game_notification" is the only option. When set to true, the alert notifying in-game users that their browser is about to open will not display. 
* Add to [overwolf.utils.enums.eStorePage](utils#estorepage-enum) - Now you can Open the requested app’s reviews/feedabck page in the Overwolf Appstore.
* [overwolf.windows.getCurrentWindow()](windows#getcurrentwindowcallback) now returns also the monitor id.  
  Now you quickly determine which monitor the window is displayed.
* New manifest flag - [process_name](manifest#process_name-notes).  
...
* [Disabled Dev Tools as a default](/topics/best-practices/enable-dev-tools#supported-features).  
  From now on, when clicking Ctrl+Shift+I on a window will NOT open the developer tools. (Even if the [disable_dt](manifest#disable_dt) manifest flag is set to false). The only way to enable it back is by changing a registry key.  
...
* [Minecraft](games/events/minecraft)
  * New supported game
* [LOL](games/events/lol)
  * New updates under:
    * match_info
* [MTGA](games/events/mtga)
  * New updates under:
    * game_info
* [LoL Launcher](games/launchers/events/lol)
  * New updates under:
    * clash

## Version 0.149 (July 2020)
...
* "DPI-aware" feature for [native](/topics/using-windows-api/windows-types#native-window) windows:  
  ...
  There's also a new manifest flag to disable that behavior if the developer wants none of that: [disable_auto_dpi_sizing](manifest#disable_auto_dpi_sizing).
...
* New feature: [overwolf.window.flash()](windows#flashwindowid-behaviorcallback)  
...
* New manifest flag: [disable_cleanup](manifest#disable_cleanup).  
  If set to true, app local data will not be cleaned up after app uninstallation.
* New API: [overwolf.extensions.io](extensions/io)  
  No need to use the external I/O plugin anymore for your current extension-related I/O functionalities.  
  Now you can easily create, delete, or update files on your extension's [dedicated storage space](extensions/io#storagespace-enum).   
...
* [Rainbow Six: Siege](games/events/rainbow-six)
  * New feature:
    * me
* [Escape From Tarkov](games/events/escape-from-tarkov)
  * New supported game
* [VALORANT](games/events/valorant)
  * New supported game
...
## Version 0.145 (April 2020)

Note that iteration 144 and 145 were combined into a single iteration.

#### API and Platform

* New API: [overwolf.campaigns.crossapp](campaigns/crossapp) for the new LoL "Refer a friend" feature. **Note: the docs for this API will be released soon**
* Cookie Sync - Cookie Syncing was updated to support Microsoft Edge (non/dev version) and the new Firefox schema.
* Security and bug Fixes.

## Version 0.143 (March 2020)
...
* New API - [overwolf.settings.language](settings/language).  
  Returns an object with language methods and events.
  * [overwolf.settings.language.get()](settings/language#getcallback).
  * [overwolf.settings.language.onLanguageChanged](settings/language#onlanguagechanged) event.
* New event: [overwolf.windows.onScreenPropertyChanged](windows#onscreenpropertychanged).  
...
* Allow overriding recording indication type and position in API:  
  Now when calling [overwolf.streaming.start()](streaming#startsettings-callback), [indication_position](streaming#indication_position-enum) and [indication_type](streaming#indication_type-enum) can be set under video settings. (Until now, you could do that only from the OW client UI).
...
* [World of Warcraft game events](games/events/wow)
  * New supported game
* [Rocket League game events](games/events/rocket-league)
  * New info updates:
    * server_info
    * mutator_settings
    * arena

## Version 0.142 (February 2020)
...
## Version 0.140 (December 2019)

* Internal bug fixes and refactoring.

## Version 0.139 (December 2019)
...
* New Event: [overwolf.extensions.onExtensionUpdated](extensions#onextensionupdated). Notify when the app was updated.  
...
* [overwolf.tobii](deprecated/tobii): this API has been deprecated.
* [League of Legends Events](games/events/lol)
  * New event:
    * match_clock
* Dota 2 Events
...
* [MTGA Events](games/events/mtga) - New supported game
   * New info-update:
...
* New manifest flag: [minimum-gs-version](manifest#meta-minimum-gs) - Allows extensions (apps) to set a minimum game summary app version in the manifest, this works similarly to minimum-version and relevant only for apps thay using the game summary pannel.
* New manifest flag: [service_providers](manifest#service_providers) - you can use this flag to set different parameters that are relevant for a service provider app. Use the new API function [overwolf.extensions.getServiceConsumers()](extensions#getserviceconsumerscallback) to read the flag value.
* New API function: [overwolf.utils.openStore()](utils#openstoreparam) - Opens the requested app’s profile/login/subscription page in the Overwolf Appstore.
* New API namespace: [overwolf.extensions.sharedData](extensions/shared-data) - a new namespace that allows any app to share data with another app. the plan is to use it for the Game Summary tabs.
...
* [minimum-gep-version](manifest#meta-minimum-gep) - New manifest flag. Allow extensions to set a minimum GEP version in manifest, this works similarly to minimum-version.

* [overwolf.extensions.updateExtension()](extensions#updateextensioncallback) - new method. Tries to download an update for the calling extension.

* [overwolf.extensions.onExtensionUpdateStateChanged](extensions#onextensionupdatestatechanged) - new event. Notify when the app was updated.

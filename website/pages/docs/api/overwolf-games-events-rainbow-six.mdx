...
* suffix - If the user use a nickname (from the privacy menu), this field will include a number between 2-5 digits. According to Ubisoft privacy rules, in case there is a value in this field it must be linked to the name field wherever it appears during the match. The suffix will be reset in the end of each match and regenerate a new one in the beginning of the next match. 

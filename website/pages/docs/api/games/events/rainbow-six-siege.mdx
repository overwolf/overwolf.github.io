...
It's highly recommended to communicate errors and warnings to your app users. Check game event status [here](/status/all) or easily check game event status directly from your app [using our API](/topics/using-events/how-to-check-events-status-from-app).
...
health | players | Player’s current health – (0-100) or `null`<br></br>`{"player":{"health":"70"}}` |   |   123.0 |

...
```json
{"info":{"match_info":{"arena_score":"1 : 2"}},"feature":"match_info"}
```
...
assist | victimName  | Local player participated in a team member’s kill.<br></br><br></br>Provided values:<br></br>1. victimName<br></br>2. type - knockdown / elimination<br></br><br></br>Example:<br></br><br></br>`{"victimName":"VioletAlbicocca","type":"knockdown"}` |                    |    130.0      | 
...
death | null (in the future we will provide the killer name). | Local player died during knocked out state. | There is another health bar during knocked out<br></br>state, it’s orange, above the player name. See [notes](#death-note) |     128.0      | 

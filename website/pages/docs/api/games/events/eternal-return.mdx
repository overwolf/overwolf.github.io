...
<GameInfo gameID={21672} showCompliance={true} showDocs={false}/>
...
team's info update does not work for premade teams.<br></br>
User number will retrun 0 as value if there are none.<br></br>
...
"roster" includes information of all 18 players in the game. <br></br>
...
This is triggered when the local player is inquired whether to accept or decline the current matching.<br></br>
...
This event cannot be used to find out the final safe area. Final safe area must be acquired by the "new_restriction" event. <br></br>
The final_restriction will be called each step of the announcement. <br></br>
Possible steps
...
#### *area_code* note
Area codes are represented as following. (Note that changes can occur, and this should be double checked via ER Open API.)<br></br>
...
Mastery codes are represented as following.<br></br>
...
Weapon codes are represented as following. (Mastery types share some common values.)<br></br>

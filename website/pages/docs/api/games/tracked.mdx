...
> If both `tracked` and `track_all_games` are set to true in the [manifest](../manifest#launch_event_settings-array), returns an object with information about the most recently launched game, similarly to the way [overwolf.games.getRunningGameInfo()](.#getrunninggameinfocallback) works.

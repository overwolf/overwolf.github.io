...
When you call [overwolf.games.onGameInfoUpdated](.#ongameinfoupdated) you will get the same full ID including the instance ID.  
...
Note that the [RunningGameInfo object](.#runninggameinfo-object) also has a `classId` property, which already equals the game ID.

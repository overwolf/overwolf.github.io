...
callback       | [(Result: FileResult)](.#fileresult-object) => void  | Called with the status of the request and the url to the target video                                   |
...
callback       | [(Result: FileResult)](.#fileresult-object) => void  | Called with the status of the request and the url to the output video     |
...
:::tip
To delete old gifs you can use [overwolf.media.videos.deleteOldGifs()](.#deleteoldgifskeepnewestxgbs-callback), and to delete old screenshots you can use [overwolf.extensions.io.delete()](../extensions/io#deletespace-path-callback).
:::

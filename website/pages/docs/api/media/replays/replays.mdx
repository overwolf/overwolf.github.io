...
callback                | [(Result: FileResult)](../#fileresult-object) => void    | A callback function which will be called with the status of the request                           |
...
Parameter               | Type                                 | Description                                                                                      |
----------------------- | -------------------------------------| ------------------------------------------------------------------------------------------------ |
param			        | [TobiiLayerParams](../deprecated/tobii#tobiilayerparams-object) | The Tobii layer visibility param                                           |
...
The `match_id` field from this event is equal to the `pseudo_match_id` from [match_info](../games/events/lol#match_info).
...
That works for all the supported games, except [LOL](../games/events/lol): in LOL the `match_id` from [onHighlightsCaptured](#onhighlightscaptured) is not equal to `pseudo_match_id` - it's the real RIOT match ID. So if you wanty to find the highlights of a specific LOL match, you should cross-reference this `match_id` with the `matchId` from the [matchState](../games/events/lol#matchstate).
...
Parameter         | Type                                        | Description             |
------------------| --------------------------------------------| ----------------------- |
source_type       | [overwolf.media.enums.eSourceType](.#esourcetype-enum)            |                         | 
name              | string                                      |                         | 
...
transform         | [overwolf.media.enums.eVideoSourceTransform](.#evideosourcetransform-enum)  |                         | 

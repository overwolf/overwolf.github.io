...
#### Game events
* Rainbow Six Siege
	* New "suffix" field was added in roster event. According to Ubisoft privacy rules, in case there is a value in this field it must be linked to the player's name field wherever it appears during the match - More details [here](overwolf-games-events-rainbow-six#roster_xx-notes)

* Axie Infinity origin
	* New game support - More details [here](overwolf-games-events-axie)

* Valorant
	* New map support for Pitt - Pearl. More details [here](overwolf-games-events-valorant)

* Dota 2
	* New support for Hero Pool detection for Captain’s Draft [here](overwolf-games-events-dota2)
...
#### Platform
- Added the `tray_icon` flag to the [manifest](../api/manifest-json). When provided, this icon will be used as the tray icon, rather than the launcher icon.
...
- Fixed issue where [`overwolf.utils.openUriInDefaultBrowser('ms-settings:display-advancedgraphics')`](../api/overwolf-utils#openurlindefaultbrowserurl) would not open the Microsoft Display settings.
- Fixed a bug involving [`overwolf.windows.flash()`](../api/overwolf-windows#flashwindowid-behaviorcallback).
- Fixed a bug where opening a native window would cause games to lose focus.
- Fixed a bug where calling [`overwolf.windows.bringToFront(false, callback)`](../api/overwolf-windows#bringtofrontgrabfocus-callback) or [`overwolf.windows.bringToFront(windowId, false, callback)`](../api/overwolf-windows#bringtofrontwindowid-grabfocus-callback) would cause a game to lose focus.
...
<div>{/* * [World of Warcraft](overwolf-games-events-wow)
...
* Fix [overwolf.utils.openUrlInOverwolfBrowser](overwolf-utils#openurlinoverwolfbrowserurl): allow to open tab and redirect back to overwolf-extension when origin URL is at 'externally_connectable' and to tab was open from the same extension.
...
* [overwolf.streaming.start](overwolf-streaming#startsettings-callback) now support undefined settings.
* WebCam support: 
...
* [Final Fantasy XIV](overwolf-games-events-ff-xiv)
  * New info update under:
    * match_info
...
* [MTGA](overwolf-games-events-mtga)
  * New updates under:
    * game_info
* [LoL Launcher](overwolf-games-launchers-events-lol)
  * New updates under:
    * clash
...
* New API: [overwolf.campaigns.crossapp](overwolf-campaigns-crossapp) for the new LoL "Refer a friend" feature. **Note: the docs for this API will be released soon**
...
* New API - [overwolf.settings.hotkeys](overwolf-settings-hotkeys).  
...
* [League of Legends Events](overwolf-games-events-lol)
  * New event & info:
    * live_client_data
* [Path of Exile game events](overwolf-games-events-path-of-exile)
  * New events & info
    * kill
    * death
    * match_outcome
...
* New API namespace: [overwolf.extensions.shareddata](overwolf-extensions-sharedData) - a new namespace that allows any app to share data with another app. the plan is to use it for the Game Summary tabs.

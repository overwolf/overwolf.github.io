    ...
    "data": {
        ...
        "windows": {
            "windowName": {
                ...
                "disable_hardware_acceleration": false, // disable GPU hardware acceleration. Relevant only to native windows
...
<SkipMe> TODO: Add link to explanation about app log file rotation </SkipMe>
<SkipMe> max_rotation_log_files was added in `0.154` </SkipMe>
...
| <a className="anchor" aria-hidden="true" id="popup_blocker"></a>popup_blocker           | bool       | Prevents new browser windows being opened automatically using script. <br/>*Default value is false*                              | 0.103  |
...
* In addition, you can set the auto-launch options using the [overwolf.settings.setExtensionSettings()](settings#setextensionsettingsextensionsettings-callback) API.
...
Note that some of the games (e.g [CS:GO](games/events/counter-strike-global-offensive)) can run in "trusted mode," which disable the "exclusive mode" in the game and makes it impossible to  interact with your OW app window.
...
Aside from that, prints attributed to `webpack` (for example, from a react dev server) cannot be whitelisted from filtering (you can check the attribution through the link at the right side of the print's line in the [dev tools console](/start/getting-started/using-dev-tools#console)).
:::

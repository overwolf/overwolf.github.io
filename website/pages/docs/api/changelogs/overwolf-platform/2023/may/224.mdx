...
- Updated the underlying `OBS` recording engine version (27.2.3 -> 29.1.0). Notable changes include:
  ...
  - Added three new `HEVC` encoders - `NVIDIA_NVENC_HEVC`, `INTEL_HEVC`, `AMD_AMF_HEVC`
    - These new encoders can only be used for recordings, and can't be played without specialized software (**meaning they won't work within an in-app video component**)!
  - Various changes to the lists of supported video encoder presets - the full list for comparison will be published in the coming days
...
- Improved the [`overwolf.social.twitter.share()`](/api/social/twitter#sharetwittershareparameters-callback) and [`overwolf.social.twitter.shareEx()`](/api/social/twitter#shareextwittershareparameters-callback-callback) methods' error handling:

...
- This spec is a general guideline for any Game Event implementations from now on to follow. Due to technical limitations, some implementations might stray away from it.
...
- Game Info should mostly be used for exposing any data of actual importance from the game.
    ...
    - Game Info exposing data of specific Game Events should be occassionally, when a predefined set of declared conditions occur, in order to avoid clutter/resource hogging.
- Not all game data should be exposed as events, but events **can** be used to notify an app of external data sources (files, external APIs, etc).
- Events planning can be tricky, with many different approaches being valid. As such, do not hesitate to [contact us](/support/contact-us) for help with it.
:::
...
Game events offer a way for an app to know the exact moment at which something happened, even before all of its data is ready/exposed to the app/player.

### Event Contents

Game events should rarely contain any concrete data. Instead, they should contain a Unique Identifier (UID) of the specific event, which will be used
to correlate it to a specific [Game Info][game-info].

- For apps, this makes it possible to initialize mid-match and still have data, even for things like .
- For games, this makes it possible to easily expose new information about old occurances as it becomes available to the player.

:::tip Game Event UID
A "full" Game Event Unique ID is made up of two parts:
- The event's type - `event_name`
- A unique ID for this specific event - `uid`

As a result, every ID'd event only needs to send the event's `uid` inside of its data.

The event `uid` should be consistently formatted throughout the entire game where possible, and should always follow one of the following patterns:
...
```json title="A player just scored the first point in the game"
{
    "name": "scored",
    "data": {
        "event_id": "scored_0"
...
:::info Event Data
While usually undesirable, game events **can** contain concrete data directly, without using an ID and a Game Info.

This should usually be avoided, for several reasons:
- It adds a different method of data access/processing compared to the rest of the specification, complicating the code for both apps and games.
- It makes any app that misses the event for one reason or another (mostly due to connecting after it occurs) unable to access this data at all.
- It makes it impossible to update this information easily in the future (with new/different data).

As such, the main use-cases for it include:
- Meta notifications - An external resource the app might cache has changed/is about to change (ie. game API keys, game files, etc).
- Fleeting unimportant (for apps) occurances - Cutscene started, player interacted with cosmetic environment, etc.
:::
...
- Game Event data - For Game Info that corresponds to a certain Game Event:
    ...
    - The Game Info name will always correspond to the remaining "tail" of the event UID.
...
- General Game Info - For general Game Info that is not related to any specific Game Event.
    ...
    - The Game Info name will always correspond to the Game Info name given to it under its relevant feature.
...
{
    "category": { // category 1
        "item_info": "Example string", // info 1
        "reset_info": null // info 2
    },
    "game_event": { // category 2
        "event_info": { // info 3
            "final": false,
            "initial_timestamp": 1671096768164,
            "inner_foo": "value",
            "inner_bar": true
        }
    },
    "local_player": { // category 3
        "number_info": 125 // info 4
        "boolean_info": true // info 5
    }
}
```
...
```json title="The blue team has just scored 5 points, making this the 3rd time they have scored in the game"
{
    "scored": {
        "blue-3": {
            "initial_timestamp": 1671097454324,
            "final": true,
            ...
        }
    }
...
```json title="The blue team has just scored 5 points, increasing their score count from 8 to 13, while red is still at 18"
{
    "match_info": {
        "score": {
            "blue": 13,

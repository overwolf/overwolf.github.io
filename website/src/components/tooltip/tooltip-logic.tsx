...
export const handleMegaToolTipOnOver = (e) => {
  ...
  let toolTipParent = e.target as HTMLTextAreaElement;
  let position = toolTipParent.getAttribute('tooltip-position');
  let toolTipSize = toolTipParent.getAttribute('tooltip-size');
  ...
  let toolTipCords = toolTipParent.getBoundingClientRect(), left, top;
  ...
  switch (position) {
    ...
    case "right":
      ...
      top = (Number(toolTipCords.top) + Number(toolTipCords.bottom)) / 2 - tooltip.offsetHeight / 2;

      if (Number(toolTipCords.right) + tooltip.offsetWidth > document.documentElement.clientWidth) {
    ...
    case "bottom":
      ...
      left = Number(toolTipCords.left) + ((toolTipCords.width - tooltip.offsetWidth) / 2);
  ...
  let availableWidth = windowRightEdge - left; // distance from window right edge
  if (availableWidth < toolTipRect.width) { // when there isn't enough space for the tooltip
    tooltipPointerOffsetX = toolTipRect.width - availableWidth;
    left = left - tooltipPointerOffsetX;
  }

  let availableHeight = windowBottomEdge - top; // distance from window bottom
  if (position == "bottom" && availableHeight < toolTipRect.height) { // when there isn't enough space below and the tooltip opens down
  ...
  if ((position == "left" || position == "right") && availableHeight < toolTipRect.height) {

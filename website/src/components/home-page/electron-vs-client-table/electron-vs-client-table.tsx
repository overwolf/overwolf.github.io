...
const ElectronVsClientTable: FC<ElectronVsClientTableProps> = (props) => {
  const { noPaddingTop = false, openByDefault = false } = props;

  // view type true = native false = electron
  const [viewType, setViewType] = useState(true);
  const [elmRole, setElmRole] = useState('');
  const [showHideTable, setShowHideTable] = useState(openByDefault);
  ...
  return (
    <>
      ...
      >
        <div
          className={`evc-inner ${showHideTable ? 'is-open' : ''}`}
          ref={tableContainer}
        >
          <div className={`evc-table ${noPaddingTop ? 'no-padding-top' : ''}`}>
            ...
            <h3 className="group-title">
              Performance
              <OWToolTipIcon text="All performance benchmarks were measured using an `AMD Ryzen 5 5600X 6-Core Processor`, and include the total overhead for running a simple App displaying a simple window." />
            </h3>
            <EVCrowGroup

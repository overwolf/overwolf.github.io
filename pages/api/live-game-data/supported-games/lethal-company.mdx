...
in_game_player_name - local player name as displayed inside a match, this parameter will update once the player entered a match.
...
legendSelect_X| match_info | The name of the legend & order of selection for every member in the team, including noting who the jump-master and local-player are. | See [notes](#legendSelect_X-note)|  128.0 |
...
```json
{"info":{"match_info":{"roster_1":"{\"name\":\"HelloWork\",\"isTeammate\":true,\"team_id\":3,\"platform_hw\":2,\"platform_sw\":7,\"state\":\"knocked_out\",\"is_local\":\"1\",\"platform_id\":\"7656119934534254\",\"origin_id\":\"2351105644\"}"}},"feature":"roster"}
```
...
* `platform_hw` / `platform_sw` - The platform which the player plays on currently.
...
* "platform_hw":2,"platform_sw":2 = PC/Origin/EA App
...
assist | victimName  | Local player participated in a team member’s kill. |                    |    130.0      | 

#### *kill* note

The data includes the victim name as parameter 
...
The data include values: 
1. The vcictime name as victimName
2. Assist type as type elimination
...
death | null | Local player died during knocked out state. | There is another health bar during knocked out<br></br>state, it’s orange, above the player name. See [notes](#death-note) |     128.0      | 

...
Match score info-update is presenting the score as it is in the game's UI, the local player is always in the blue team
...
```json
{"feature":"roster","category":"players","key":"roster_0","value":"{\"player\":\"Overwol\",\"is_anonymous\":false,\"team\":\"Blue\",\"kills\":0,\"deaths\":1,\"score\":-415,\"assists\":0,\"ping\":1, \"operator\":\"SLEDGE\"}"}
```
...
* is_anonymous - Boolean if the player's privacy it set to true or false
* team - Blue/Orange
...
Important note - If the local player is in a party/squad with friends, the account_id_log may sometimes contain information pertaining to the party members instead. It is not possible to differentiate, so suggest listening for the very first event upon game launch (before local player joins a squad), and then ignoring all future events.
